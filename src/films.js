...
function moviesAverageOfDirector(array, director) {
  const moviesFromDirector = array.filter((item) =>
    item.director.includes(director)
  );
  const scoreFromTheMovies = moviesFromDirector.map((item) => item.score);
  const sumScores = scoreFromTheMovies.reduce((prev, curr) => prev + curr, 0);
...
function moviesAverageByCategory(array, genre) {
  ...
  const moviesCategory = moviesScores.filter((item) =>
